...
<head>
    ...
    <title>Data Permintaan Layanan - Lab Rawa</title>
    <!-- Bootstrap 5 CSS -->
    ...
    <style>
        ...
        .custom-select { border: 1px solid #333; border-radius: 8px; padding: 0 15px; height: 35px; font-weight: 600; color: #345E6F; font-size: 14px; width: 150px; outline: none;}
        ...
        .table-custom td { padding: 15px 10px; text-align: center; border-bottom: 1px solid #333; vertical-align: middle; }
...
<div class="main-wrapper">
   ...
   <div class="topbar-card shadow-sm">
    ...
    <div class="dashboard-container">
        ...
        <!-- Toolbar Section (Search, Status, Edit, Hapus) -->
        <div class="toolbar-card">
            <div class="toolbar-title">Data Permintaan Layanan</div>
            <div class="d-flex justify-content-between align-items-end mt-2">
                
                <!-- Kiri: Search, Status, Print -->
                <div class="d-flex flex-column gap-3">
                    <form method="GET" class="d-flex gap-2" style="align-items: center;">
                        <div class="search-wrapper">
                            ...
                            <input type="text" name="search" class="search-input" placeholder="Cari permintaan..." value="{{ $search ?? '' }}">
                        </div>
                        <button type="submit" class="btn-custom btn-print" style="width: 50px; padding: 0;">
                            <i class="bi bi-search"></i>
                        </button>
                    </form>
            ...
            <table class="table-custom">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-{{ $permintaan->status === 'selesai' ? 'success' : ($permintaan->status === 'diverifikasi' ? 'info' : 'warning') }}">
...
<!-- Script Bootstrap (Opsional jika butuh dropdown/interaktivitas lain) -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
<script>
    function deleteSelected() {
        const checkboxes = document.querySelectorAll('.form-check-input-custom:checked');
        if (checkboxes.length === 0) {
            alert('Pilih permintaan yang ingin dihapus');
            return;
        }
        
        if (confirm('Yakin ingin menghapus permintaan yang dipilih?')) {
    ...
    document.getElementById('statusFilter')?.addEventListener('change', function() {
        if (this.value) {
            window.location.href = `{{ route('kepala.permintaan') }}?status=${this.value}`;
        } else {
            window.location.href = `{{ route('kepala.permintaan') }}`;
        }
    });
</script>
</body>
