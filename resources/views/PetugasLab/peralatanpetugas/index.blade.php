...
<head>
    ...
    <style>
        ...
        /* Search Elements */
        .search-wrapper { display: flex; align-items: center; border: 1px solid #333; border-radius: 6px; overflow: hidden; height: 35px; width: 350px; }
        ...
        .btn-edit { background-color: #B2C3CF; color: #333; }
        .btn-edit:hover { background-color: #9cb1c0; }
...
<div class="main-wrapper">
    ...
    <div class="dashboard-container">
        ...
        <div class="toolbar-card">
            ...
            <form method="GET" action="{{ route('petugas.peralatan.index') }}" class="d-flex flex-column gap-3 mt-3">
                
                <!-- Baris Atas: Search Box -->
                <div class="search-wrapper">
                    <div class="search-icon-box">
                        <i class="bi bi-search"></i>
                    </div>
                    <input type="text" name="search" value="{{ $search ?? '' }}" class="search-input" placeholder="Cari kode BMN, nama, status">
                </div>

                <!-- Baris Bawah: Dropdown & Buttons -->
                <div class="d-flex justify-content-between align-items-center">
                    <!-- Kiri: Dropdown Status -->
                    <select name="status" class="custom-select text-muted">
                        <option value="">Status Kalibrasi</option>
                        <option value="sudah dikalibrasi" @selected(($status ?? '') === 'sudah dikalibrasi')>Sudah Kalibrasi</option>
                        <option value="belum dikalibrasi" @selected(($status ?? '') === 'belum dikalibrasi')>Belum Kalibrasi</option>
                    </select>

                    <!-- Kanan: 3 Tombol Aksi -->
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn-custom btn-tambah">Cari</button>
                        <a href="{{ route('petugas.peralatan.create') }}" class="btn-custom btn-edit">Tambah</a>
                    </div>
                </div>
            </form>
        </div>
